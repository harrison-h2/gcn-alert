...
def _fetch_event_data(event_id, query_param="trigger_id"):
    """Fetch and unwrap a Colibri event response. Returns parsed dict or None on any error."""
    if not event_id:
        return None
    try:
        response = requests.get(f"{URL}/event", params={query_param: event_id}, timeout=10)
...
def lookup_event_details(event_id, query_param="trigger_id"):
    """
    ...
    """
    data = _fetch_event_data(event_id, query_param=query_param)
    if data is None:
